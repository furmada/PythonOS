...
class Network(pyos.GUI.Container):
    def __init__(self, cell):
        self.cell = cell
        super(Network, self).__init__((0, 0), color=state.getColorPalette().getColor("background"), width=fiapp.scroller.container.width, height=40,
                                      border=2, borderColor=(20, 20, 20))
        self.addChild(pyos.GUI.Image((0, 0), surface=state.getIcons().getLoadedIcon("info"), width=40, height=40,
                                     onClick=self.displayInfoDialog))
        self.addChild(pyos.GUI.Text((42, 4), self.cell.ssid, state.getColorPalette().getColor("item"), 24))
        self.connBtn = pyos.GUI.Button((self.width-50, 0), "Connected" if fiapp.currentCell==self.cell else "Connect",
                                       (100, 100, 200) if fiapp.currentCell==self.cell else (100, 200, 100), (20, 20, 20), 18,
    ...
    def connectAsk(self):
        ...
        if self.cell.encrypted:
            pyos.GUI.AskDialog("Password", "The network "+self.cell.ssid+" is encrypted using "+self.cell.encryption_type+". Enter the password.", self.connect).display()
        else:
            self.connect(None)
    ...
    def connect(self, pwd):
        try:
            try:
                ...
                scheme.save()
                scheme.activate()
            state.getNotificationQueue().push(pyos.Notification("Connected", "Wifi: "+self.cell.ssid, image=state.getIcons().getLoadedIcon("wifi"),
                                                                source=app))
            fiapp.currentCell = self.cell
            app.parameters["network"] = self.cell
            self.refresh()
        except:
            pyos.GUI.OKDialog(self.cell.ssid, "Unable to connect to "+self.cell.ssid).display()
        
    def displayInfoDialog(self):
        info = "Wireless Information\n"
        info += "SSID: "+self.cell.ssid+"\n"
        info += "Security: "+(self.cell.ssid.encryption_type if self.cell.encrypted else "None")+"\n"
        info += "Signal strength: "+str(self.cell.signal)
        pyos.GUI.OKDialog(self.cell.ssid, info).display()
